...
class Magikarp(object):
	...
	def create_dis_model(self):
		...
		self.d_pred_real = self.d_predict(tf.concat(axis=1, values=[self.person_board_1, self.person_board_2]), self.p_keep)
		self.d_pred_fake = self.d_predict(tf.concat(axis=1, values=[self.person_board_1, self.gen_board]), self.p_keep)
	...
	def train(self):
		...
		for epoch in range(self.num_epochs):
			num_batches = 1 #self.data_size/self.batch_size
			g_avg_cost = 0
			d_avg_cost = 0
