...
def negamax1(board, depth, color, alpha, beta, evaluator):
	if board.is_checkmate() or depth > MAX_DEPTH1:
		input_board = convert_board(board).flatten().reshape((1,-1))
		return (color*evaluator.get_prediction(input_board), None)
	maxval = float('-inf')
	best_move = None
	...
	for move in board.legal_moves:
		board.push(move)
		val = -1*negamax1(board, depth+1, -1*color, -1*beta, -1*alpha, evaluator)[0]
		board.pop()
		# print(val, move)
		if val > maxval:
			maxval = val
